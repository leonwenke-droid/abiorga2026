...
export default async function AdminShiftsPage({
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      ...
      <h1 className="mt-4 text-2xl font-bold text-cyan-100">Schichten – {org.name}</h1>
      <p className="mt-1 text-sm text-cyan-300">Planen (Jahrgang)</p>
      <ul className="mt-6 space-y-2 rounded-lg border border-cyan-500/30 bg-card p-4">
        ...
        {(!shifts || shifts.length === 0) && (
          <li className="text-cyan-400/80">Noch keine Schichten.</li>
        )}
      </ul>
    </div>
  );
