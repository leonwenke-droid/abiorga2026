...
async function sendInviteViaN8n(
  ...
  if (!actionLink || typeof actionLink !== "string") {
    return { error: "Einladungslink konnte nicht erzeugt werden." };
  }

  const subject = "Einladung als Komiteeleitung – AbiOrga";
  const body =
    (fullName ? `Hallo ${fullName.trim()},\n\n` : "Hallo,\n\n") +
    "Sie wurden als Komiteeleitung (Lead) eingeladen. Klicken Sie auf den folgenden Link, um Ihr Konto zu aktivieren und ein Passwort zu setzen:\n\n" +
    actionLink +
    "\n\nNach dem Setzen des Passworts werden Sie automatisch zum Admin-Dashboard weitergeleitet.\n\nMit freundlichen Grüßen\nIhr AbiOrga-Team";
  ...
  if (!webhookRes.ok) {
    return { error: "E-Mail konnte nicht versendet werden. Bitte später erneut versuchen." };
  }
...
export async function setMemberAsLeadAction(
  ...
  if (!emailTrimmed) return { error: "E-Mail ist für Komiteeleitung erforderlich." };
...
export async function addMemberAction(
  ...
  if (options?.asLead && emailTrimmed) {
    ...
    if (inviteResult.error) {
      revalidatePath(`/${orgSlug}/admin/members`);
      return { error: `Mitglied angelegt, aber Einladungs-Mail konnte nicht gesendet werden: ${inviteResult.error}` };
    }
  }
